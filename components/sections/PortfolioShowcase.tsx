    ...
    <section id="portfolio-showcase" className="relative z-10 px-6 py-24 sm:py-32">
      <div className="mx-auto max-w-6xl">
        ...
        <Reveal delay={100} className="flex justify-center overflow-x-auto px-6">
          <div className="relative inline-flex shrink-0 items-center gap-0.5 rounded-full border border-line bg-veil p-1.5 sm:gap-1">
            {TABS.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`relative z-10 flex min-h-[44px] items-center gap-1.5 whitespace-nowrap rounded-full px-3 py-2 text-xs font-semibold transition-[color,transform] duration-200 active:scale-[0.97] sm:gap-2 sm:px-5 sm:text-sm ${
                    isActive ? "text-on-accent font-bold" : "text-dim hover:text-ink"
                  }`}
                >
                  {isActive && (
                    <motion.span
  ...
  return (
    <div>
      ...
      <div className="mb-10 flex justify-center overflow-x-auto px-6">
        <div className="inline-flex shrink-0 items-center gap-4 border-b border-line sm:gap-9">
            ...
            return (
              <button
                ...
                className={`relative flex min-h-[44px] items-center gap-1.5 whitespace-nowrap px-0.5 pb-3 text-xs font-semibold transition-colors duration-200 sm:gap-2 sm:text-sm ${
                  isActive ? "text-accent-strong" : "text-dim hover:text-ink"
                }`}
              >
                ...
                <span
                  className={`rounded-full px-1.5 text-[11px] font-bold tabular-nums transition-colors ${
                    isActive ? "bg-accent/15 text-accent-strong" : "bg-veil-2 text-dim"
                  }`}
                >
                  {counts[seg.id]}
                </span>
      ...
      {CERTIFICATES.map((cert) => (
        ...
        >
          ...
          <div className={`relative aspect-[4/3] overflow-hidden border-b border-line bg-black ${cert.imageFit === "contain" ? "bg-surface" : ""}`}>
            ...
            <span className="absolute right-3 top-3 inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-black/70 px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.15em] text-white backdrop-blur-sm transition-colors group-hover:border-accent/50">
          ...
          <div className="flex items-start gap-3 p-6">
            ...
            <div>
              <h3 className="text-base font-bold leading-snug text-ink">{cert.name}</h3>
              <p className="mt-1 text-sm text-dim">{cert.org}</p>
              {cert.date && (
                <p className="mt-1 font-mono text-xs text-dim">{cert.date}</p>
              )}
            </div>
          </div>
        </motion.a>
...
function TechStackTab() {
  ...
  return (
    <div className="flex flex-col gap-16">
      ...
      {/* Proficiency legend */}
      <Reveal className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 border-t border-line pt-8">
        <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
          Proficiency
        </span>
        {LEVEL_ORDER.map((lvl) => (
          <span key={lvl} className="flex items-center gap-2">
            ...
            />
            <span className="text-xs text-dim">{lvl}</span>
          </span>
        ))}
      </Reveal>
    </div>
